...
import {
  type CreateProductVariantFormData,
  createProductVariantSchema,
} from '@/features/admin/validator';
import type { IMediaItem, IProductItem } from '@/types';

interface ProductVariantFormProps {
  defaultValues?: Partial<CreateProductVariantFormData>;
...
export function ProductVariantForm({
  ...
  const products: IProductItem[] = (productsData?.data?.items ?? []).sort((a, b) =>
    a.name.localeCompare(b.name),
  );

  function handleSubmit(data: CreateProductVariantFormData) {
    onSubmit(data, form);
  }
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base font-medium">Variant Details</CardTitle>
      </CardHeader>
      <Separator />
      <CardContent className="pt-4">
        <form id="product-variant-form" onSubmit={form.handleSubmit(handleSubmit)}>
          <FieldGroup>
                ...
                render={({ field, fieldState }) => (
                  <Field data-invalid={fieldState.invalid || undefined}>
                    ...
                    >
                      ...
                      <SelectContent>
                        {Object.values(EProductVariantSize).map((size) => (
                ...
                render={({ field, fieldState }) => (
                  <Field data-invalid={fieldState.invalid || undefined}>
                    ...
                    <Input
                      {...field}
                      id="sku"
                      placeholder="e.g. WH-1000XM5-BLK-M"
                      autoComplete="off"
                      disabled={isPending || isLoading}
                    />
                    {fieldState.invalid && <FieldError errors={[fieldState.error]} />}
      ...
      <CardFooter className="gap-2">
        <Button type="submit" form="product-variant-form" disabled={isPending || isLoading}>
