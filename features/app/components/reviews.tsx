...
import { Calendar, MessageCircle, Send, Star, ThumbsUp } from 'lucide-react';
              ...
              <CardContent>
                ...
                <div className="mt-4 flex gap-4">
                  <Button
                    ...
                    size="sm"
                    onClick={() =>
                      setIsLiked((prev) => ({ ...prev, [review.id]: !prev[review.id] }))
                    }
                    className="text-muted-foreground hover:bg-primary/10 hover:text-primary h-8 cursor-pointer px-3 text-xs"
                    aria-label={`${isLiked[review.id] ? 'Unlike' : 'Like'} this review`}
                  >
                    <ThumbsUp className={cn('size-4', isLiked[review.id] && 'fill-current')} />
                  ...
                  <div className="flex gap-1" role="radiogroup" aria-label="Rating">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <Button
                        ...
                        className="focus-visible:ring-ring h-9 cursor-pointer rounded-full !p-1 px-4 py-2 hover:bg-transparent focus:outline-hidden focus-visible:ring-2"
                      ...
                      >
                        <Star
                          className={cn(
                            'size-6',
                            (hoveredStar || rating) >= star ? 'fill-foreground' : '',
                          )}
                        />
                      </Button>
                    ))}
